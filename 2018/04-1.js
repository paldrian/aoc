var inputs = ["[1518-04-22 00:56] falls asleep",
...
inputs.forEach(function(input){
    ...
    if(input.indexOf('Guard')!=-1){
        guard = input.substring(input.indexOf('#'));
        guard = parseInt(guard.substring(1,(guard.indexOf(' '))));

        if(!output[guard])
            output[guard] = {max:0};


    }else if(input.indexOf('falls')!=-1){
        start = parseInt(input.substring(input.indexOf(']')-2,input.indexOf(']')));

    }else{
        var end = parseInt(input.substring(input.indexOf(']')-2,input.indexOf(']')));

        if(!output[guard]['sleep']) output[guard]['sleep'] = 0;

        output[guard]['sleep']+=end-start;

        if(maxsleep<output[guard]['sleep'])
        {
            maxsleep = output[guard]['sleep'];
            selectedguard = guard;
            console.log('guard #'+guard+' sleeps '+output[guard]['sleep']);
        }
        ...
        for(var i = start; i<end; i++){
            ...
            if(maxsleep===output[guard]['sleep'] )
            {
                console.log('guard #'+guard+' sleeps '+output[guard][i]+' times on minute '
                +i +':'+guard*i);
            }
        }
...
for(var i=0; i<59; i++){
    ...
    if(output[selectedguard][i] && output[selectedguard][i]>max)
    {
        max = output[selectedguard][i];
        console.log('guard #'+selectedguard+' sleeps '+output[selectedguard][i]+' times on minute '
                +i +':'+selectedguard*i);
    }
}
